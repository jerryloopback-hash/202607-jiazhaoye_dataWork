...
| 项 | 原规划 | 现方案 |
|---|---|---|
| 场景B方法 | 图嵌入 node2vec（ER 关系 → 相似人群/兴趣类群） | **XGBoost(或其轻量变体) 流失预测**（另有 lr/rf/rule 路由对比） |
...
- **方法**：XGBoost（主方法；路由程序另备 逻辑回归/随机森林/RFM规则评分卡 三方法一参切换对比）。
...
- 算法细节：XGBoost early-stopping + `scale_pos_weight` 类别平衡；lr/rf 用 `class_weight="balanced"`；rule 按 R最近行为(0.4)/F频次(0.35)/M金额(0.25) 分箱加权打分（分箱/权重/分档阈值可经 `scripts/sceneB_rfm_tuner.py` 命令行传参调参，免改代码）。
...
### 3.5 增量优化（离线批量 + 增量重训）
- **批量打标**：每日/每周调度，`--mode predict` 读 DWS 特征宽表 → 用上线模型全量打标（秒级，不重训）。
- **增量重训**：新样本随窗口滚动累积；按固定节奏(如每周)用"历史累积 + 新增"**全量重训并版本化**，
  保留旧模型可回滚；或用已成熟标签做 warm-start 微调（可扩展）。
...
| 评估报告 | `output/sceneB_eval_report.txt` | AUC/PR-AUC/混淆矩阵/分类报告/特征重要性 |
| 对比报告 | `output/sceneB_model_comparison.txt` | 四方法同台对比（--compare 时） |
| 模型(版本化) | `output/model/model_<方法>_<ts>.json/joblib` + `latest.json` | 指针可回滚 |
...
> 服务器部署、环境依赖、建表回填、调度编排、监控回滚的**完整操作步骤**见 **[生产部署运行手册](sceneB/生产部署运行手册.md)**；逐步操作与输入输出格式见 **[使用说明](sceneB/使用说明.md)**；环境安装复刻见 **[环境安装指南](sceneB/环境安装指南.md)**。
...
| 上线机制 | `--deploy` 显式切换（2026-09-02） | 训练/对比只产新版本、不自动改 latest.json，杜绝测试性训练覆盖线上模型；2026-09-03 起 train 模式支持 `--deploy true` 训练后一步上线（`--compare`/predict 模式不支持，传入即报错） |
| RFM 调参方式 | 独立调参脚本传参（2026-09-03） | `sceneB_rfm_tuner.py`：分箱/权重/分档阈值命令行传入或 `--auto` 自动调参（分位数分箱+权重网格搜 AUC），可按目标占比定阈值；产出 rule 版本、`--deploy` 上线，运营免改代码 |
| 增量/输出 | 离线批量 + 增量重训 | 标签表版本化，可回滚可回溯；`--mode predict` 日常打标不重训 |

*文档维护：2026-08-31（2026-09-01 补生产部署运行手册/使用说明/环境安装指南、本机真实跑通训练打标、四方法路由对比；2026-09-03 补 RFM 调参器 sceneB_rfm_tuner.py）。关联：[[智能打标调研/PLAN]]、[[03_场景与标签需求/场景标签与方法匹配]]、[[02_方法调研/方法流程与可行性]]、[[智能打标调研/README]]。*
