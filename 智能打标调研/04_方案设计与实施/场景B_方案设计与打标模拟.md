...
>
> **本 demo 定位**：跑通「ODS 改造数据 → 特征工程 → 自造标签 → XGBoost 训练 → 评估 → 批量打标 → 增量重训」完整生产型链路（2026-09-01 已在本机真实跑通）。
...
> 服务器部署、环境依赖、建表回填、调度编排、监控回滚的**完整操作步骤**见 **[生产部署运行手册](sceneB/生产部署运行手册.md)**；环境安装复刻见 **[环境安装指南](sceneB/环境安装指南.md)**。
