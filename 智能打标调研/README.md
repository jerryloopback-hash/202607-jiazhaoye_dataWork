...
> （分别对应 **DWS 直算类标签** 与 **ER 关系类标签**）在测试环境跑通打标流程模拟。
...
2. **两个场景 + 两类标签 + 两种方法**：
   ...
   - **场景 B（依赖 ER 关系层）**：产出【关系/相似类标签】，方法=图嵌入（node2vec 等）。
...
| 两场景原则 | 场景 A 直接吃 DWS 用户宽表（一行一用户，行为窗口特征）；场景 B 吃 ER 层的关系数据（用户×资源）。两者各配一类标签、一种方法，避免摊大饼。 |
...
- **我方**：负责【标签层】。其中场景 A 的标签可直接从 DWS 计算，场景 B 的标签需结合 ER 层关系数据计算。
...
### 📋 可选扩展（按需）
- [ ] 场景 B 方案设计占位（培训课程兴趣类群/图嵌入；需 ER 层到位后实施）
- [ ] 场景 2 流失预测方案（GBDT，需自造标签 y）
- [ ] 场景 5 高价值会员分层（RFM+聚类）
- [ ] 多场景标签效果回测方案设计
