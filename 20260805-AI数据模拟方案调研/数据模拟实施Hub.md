...
- **Persona 层架构变更**：删除全部手工固定 Persona（原 §3.3 P001-P020，共20条），改为从 Tencent PersonaHub 批量筛选改造，由本地 Ollama LLM 逐条生产，目标 500-2000 条，存储于 `data/personas/wenti_personas.jsonl`。
...
| DEC-002 | Persona 来源 | 全量从 Tencent PersonaHub 筛选改造（目标500-2000条），不手工设计固定 Persona；改造结果通过本地 Ollama LLM 逐条生产 | persona/ 模块，Prompt B 设计，hub_adapter.py |
...
数据生成流程采用**五层架构**，每层输出作为下一层输入，逐层抽象降低：
...
**输出**：
- 场馆列表（venue_id, name, province, city, area）
- 运动类型列表（sport_id, name, category）
- 优惠券模板（j_coupon_code 表记录）
- 培训课程模板（t_class 表记录）
- 时间卡模板（j_member_time_card 的 card_name/type/limit_num 枚举）

**实现方式**：
- 调用 LLM（Prompt A）生成合理的字典数据
- 存储在 `data/dict/*.json`，后续生成时作为外键参照
...
**输入**：
- Tencent Persona Hub 下载的原始数据
- Prompt B（Persona 改造模板）

**输出**：
- 500-2000 条文体化 Persona JSONL 文件（`data/personas/wenti_personas.jsonl`）
- 每条 Persona 包含：用户类型、价格敏感度、偏好场景、活跃时段、消费频次、注册渠道、生命周期、身份校验状态、行为概率矩阵

**实现方式**：
- `persona/hub_adapter.py` 下载 PersonaHub 子集（200K 条），关键词筛选后得约 800-1500 条候选
- 对每条候选记录调用本地 **Ollama LLM**（Prompt B）逐条改造为文体消费场景 Persona
- **完全不使用手工固定 Persona**；所有行为多样性均由 PersonaHub 原始描述的差异性保证

#### 第3层：行为决策层（层一 LLM）

**目的**：从 Persona 的视角生成行为决策（语义层面，不涉及具体字段）。

**输入**：
- 选定的 Persona JSON
- 当前场景描述（如："周末下午购买游泳票"）
- 第1层的字典数据摘要（可选场馆、可用优惠券等）
- Prompt C（行为决策模板）
...
```json
{
  "decision_id": "D20260807-001",
  "persona_id": "P_HUB_XXXX",
  "scenario": "weekend_swimming_ticket",
  "reasoning": "大学生健身爱好者，价格敏感，周末时间充裕，倾向选择优惠活动...",
  "actions": {
    "select_venue": {"venue_id": "V001", "venue_name": "深圳湾体育中心"},
    "select_ticket_type": {"ticket_type": 1, "ticket_name": "成人票", "quantity": 1},
    "use_coupon": true,
    "coupon_code_id": "C001",
    "bind_wechat": true,
    "payment_method": 2,
    "order_time": "2026-08-02 14:30:00",
    "complete_payment": true,
    "pay_time_offset_minutes": 3
  }
}
```

**实现方式**：
- `generators/behavior_generator.py` 调用 LLM（Prompt C）
- 使用思维链（Chain-of-Thought）要求 LLM 先推理、再输出决策
- 批量生成时可并行调用（多线程/异步）

#### 第4层：数据翻译层（层二 LLM）

**目的**：将行为决策翻译为数据库表的字段值（技术层面，直接映射为 SQL INSERT）。

**输入**：
- 第3层的行为决策 JSON
- 目标表的字段 Schema（字段名、类型、约束、枚举值）
- Prompt D（字段翻译模板）
...
```json
{
  ...
  "j_member_order": {
    ...
    "venue_id": "V001",
    "sport_id": "S001",
    "order_num": "JN20260802143000001",
    "trade_no": "WX20260802143200001",
    "type": 3,
    "cost": 50.0,
    ...
    "status": 2,
    "pay_way": 2,
    "pay_time": "2026-08-02 14:33:00",
    "create_time": "2026-08-02 14:30:00",
    "order_time": "2026-08-02 14:30:00",
    "is_pay": 1,
    "is_refund": 0,
    "merchant_id": "M001"
  ...
  "j_order_ticket": {
    ...
    "venue_id": "V001",
    "status": 0,
    "expire_time": "2026-08-09 23:59:59",
    "remain_num": 1,
    "price": 40.0,
    "service_charge": 0.0,
    "deducted_amount": 10.0
  }
}
```
...
- 强调枚举值一致性（如 sex=1 而非 "male"）
- 处理跨表外键关联（如 order_num 在三张表中保持一致）
...
**实现方式**：
- `generators/dirty_injector.py`：按照 5% 比例随机注入脏数据规则（见第6章）
...
以下为29张核心表的精确执行批次，按外键依赖关系排序：

#### 批次1：字典数据（无依赖，可并行生成）

1. `dict_venues`（场馆字典，虚拟表）
2. `dict_sports`（运动类型字典，虚拟表）
3. `dict_merchants`（商户字典，虚拟表）
4. `j_coupon_code`（优惠券模板）
5. `t_class`（培训课程模板）

#### 批次2：用户主表（依赖批次1的 venue_id）

6. `j_member`（C端会员）
7. `t_student`（培训学员）

> **跨系统用户生成顺序（M013）**：批次2中先遍历生成所有 `j_member` 记录，完成后从中筛选出 Persona 维度 `system="cross_system"` 的会员（即 PersonaHub 改造结果中 `cross_system_user=True` 的条目），复用其 `phone` 生成对应 `t_student` 记录。不得在同一循环中交替写两张表，避免 phone 池冲突。

#### 批次3：用户扩展表（依赖批次2的 member_id/student_id）

8. `j_member_third`（第三方绑定，依赖 j_member.id）
9. `buy_ticket_people`（购票人，依赖 j_member.id）
10. `j_member_time_card`（时间卡，依赖 j_member.id）

#### 批次4：优惠券发放（依赖批次1的 j_coupon_code.id 和批次2的 j_member.id）

11. `j_coupon`（用户优惠券）

#### 批次5：jianengliang 订单主表（依赖批次2的 j_member.id 和批次1的 venue_id/sport_id）

12. `j_member_order`（订单主表）

#### 批次6：training 订单主表（依赖批次2的 t_student.id 和批次1的 venue_id）

13. `m_trade_order`（培训订单主表）

#### 批次7：jianengliang 订单明细表（依赖批次5的 j_member_order.order_num）

14. `j_order_field`（订场明细，依赖 j_member_order.order_num）
15. `j_order_ticket`（游泳票明细，依赖 j_member_order.order_num）
16. `j_order_btp`（订单购票人关联，依赖 j_member_order.order_num + buy_ticket_people.id）
17. `j_order_coupon`（订单优惠券关联，依赖 j_member_order.order_num + j_coupon.id）

#### 批次8：training 订单明细表（依赖批次6的 m_trade_order.order_num）

18. `m_trade_order_course`（培训课程明细，依赖 m_trade_order.order_num + t_class.id）
19. `m_trade_order_detail`（培训订单详情，依赖 m_trade_order.order_num）
20. `t_class_teacher`（课程教练关联，依赖 t_class.id）

#### 批次9：票务核销（依赖批次7的 j_order_ticket.ticket_num）

21. `j_order_ticket_valid`（票务核销记录，依赖 j_order_ticket.ticket_num）
22. `j_time_card_use`（时间卡使用记录，依赖 j_member_time_card.card_id + j_member_order.order_num）

#### 批次10：积分记录（依赖批次5的 j_member_order.order_num）

23. `j_points_records`（积分记录，依赖 j_member.id + j_member_order.order_num）

#### 批次11：vmdb 核心表（依赖批次1的 venue_id 和批次2的 member_id）

> ✅ **表名已核实**：已对照 `ods_wenti_starrocks.sql` 完整 DDL 确认以下真实表名（2026-08-10）。5.5节字段映射已同步更新。

24. `ods_wenti_vmdb_h_member`（vmdb会员信息表，依赖 venue_id + merchant_id）
25. `ods_wenti_vmdb_h_member_card`（会员专项卡表，依赖 h_member.id）
26. `ods_wenti_vmdb_m_enter_gate`（入闸记录，依赖 venue_id + merchant_id）
27. `ods_wenti_vmdb_m_trade_order`（vmdb交易订单主表，依赖 h_member.id + venue_id）
28. `ods_wenti_vmdb_m_trade_order_ticket_verify`（散票核销，依赖 m_trade_order_ticket_no）
29. `ods_wenti_vmdb_m_venue_customer`（场馆顾客表，依赖 venue_id）

**执行策略**：
- 同一批次内的表可并行生成（多线程/多进程）
- 批次间严格串行，确保外键参照完整性
- 批次11（vmdb）可与批次7-10并行（弱依赖）
...
# 使用次数递减
j_member_time_card.remain_num = initial_limit_num - count(j_time_card_use)
```

**数据生成规则**：
- 批次3生成 j_member_time_card（部分 Persona 拥有时间卡）
- 批次9生成 j_time_card_use（随机选择时间卡用户的订单，标记为时间卡抵扣）
- 抵扣后更新 j_member_time_card.remain_num
...
**预期筛选结果**：5000条原始数据中预计筛选出 800-1500 条相关记录。
...
> 不再维护任何手工编写的固定 Persona；所有 Persona 由 `hub_adapter.py` 通过本地 **Ollama LLM** 逐条生产，存储于 `data/personas/wenti_personas.jsonl`。
...
运行 `hub_adapter.py` 完成批量改造后，`wenti_personas.jsonl` 中条目应满足以下分布约束（`hub_adapter.py` 后处理阶段自动校验，不满足则加权重采样纠正）：

| 维度 | 目标分布 |
|------|---------|
| system | jianengliang ≥50%，training 15-25%，vmdb 10-20%，cross_system 5-10% |
...
| dirty_data_probability | ≥0.10 的"高脏数据"条目占比约 5-10% |
...
由于 PersonaHub 改造后条目数量大（500-2000条），不再使用固定权重字典，改为按 `system` 字段分层抽样：
...
| remain_num | INT | 初始=quantity，使用后递减 | 非空 |
...
def main():
    ...
    # ── Batch 3-11：TODO（按 2.3 节 DAG 顺序逐批实现） ──────────────────
    # 每批逻辑：
...
不能反映真实冷启动用户分布。
缓解：`hub_adapter.py` 后处理阶段自动校验3.3节分布目标，不满足则加权重采样纠正；
Phase 1 结束后核查 `sex_dist`/`source_dist`/`consumption_frequency` 分布，
与业务方提供的历史比例（如有）做对比；若偏差 >20%，在 `config.yaml` 调整
`persona.system_weights` 后重跑。
